...
function renderModeBadge(){
  ...
  const enabled = cfg("showDemoBadge", true);
...
function renderThemes(){
    ...
    function renderFeatured(){
      ...
      $("featured").innerHTML = featuredItems.map(p=>{
        ...
        return `<article class="fcard premium" data-detail="${p.id}">
          <div class="fmedia">
            <img src="${p.image}" alt="${loc(p.name)}" loading="lazy" decoding="async" onerror="this.style.display='none';this.parentElement.classList.add('broken')">
            <div class="foverlay">
              <div class="feyebrow">
    ...
    function renderProductState(id){
      ...
      const q=S.cart[id]||0;
      const fav=isFav(id);
      document.querySelectorAll(`article.card[data-detail="${safeSelectorId(id)}"]`).forEach(cardEl=>{
...
function preserveScrollOnWide(fn) {
  ...
  accord.innerHTML=gs.map((g,idx)=>{
    ...
    return `<div class="grp ${idx?"closed":""}">
...
function openSheet(){open($("sheet"),$("sheetBack"),"sheet-open")}function closeSheet(){close($("sheet"),$("sheetBack"),"sheet-open")}
...
function getFilteredCats(){return menuData.map(c=>({...c,products:c.products.filter(match)})).filter(c=>c.products.length)}
    ...
    $("topCart").onclick=openSheet;$("floatBtn").onclick=openSheet;$("closeSheet").onclick=closeSheet;$("sheetBack").onclick=closeSheet;$("clearCart").onclick=clearCart;$("copyBtn").onclick=()=>{S.waiterView=!S.waiterView;renderCart()};
